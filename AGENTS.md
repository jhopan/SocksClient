...
- TUN defaults in `boxcfg`: stack `gvisor`, MTU `1400` (`DefaultTunMTU`), DNS `ipv4_only`. `mixed` (system TCP + gvisor UDP) and `system` are selectable; an unknown stack falls back to gvisor, never to system. Rationale: hotspot MTUs are often smaller (PMTU blackhole with big MTU) and gvisor does not depend on the machine's NIC driver stack. Keep `dumpconfig` variants in sync when these change.
